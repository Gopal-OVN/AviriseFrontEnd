...
import { ShipmentStatusDataType } from "../../../types";
...
type Props = {
  tableData: ShipmentStatusDataType[];
  refresh: (parent?: any) => void;
  isLoading?: boolean;
};

const ShipmentStatusTable = ({ tableData, refresh, isLoading }: Props) => {
  const navigate = useNavigate();
  const [loading, setLoading] = useState(false);
  const [showDeleteModal, setShowDeleteModal] = useState(false);
  const [selectedShipmentStatusId, setSelectedShipmentStatusId] = useState<
    number | null
  >(null);

  const handleEditClick = (item: any, type: any) => {
    navigate("/edit-shipment-status", {
      state: {
        data: item,
        type: type,
      },
  ...
  const confirmDelete = (id: any) => {
    setSelectedShipmentStatusId(id);
    setShowDeleteModal(true);
  };

  const handleDelete = async () => {
    if (!selectedShipmentStatusId) return;
    setLoading(true);
    try {
      const result = await deleteShipmentStatusAPI(selectedShipmentStatusId);
      refresh();
      setLoading(false);
              ...
              <tr key={index}>
                ...
                {/* <td>
                  <div className="btn-box">
                    ...
                    <button onClick={() => confirmDelete(item.shipment_status_id)}>
